...
本文档描述了为C/C++开发的Juliet测试套件v1.2。该测试套件由国家安全局（NSA）的放心软件中心（CAS）创建并开发，专门用于评估静态分析工具的能力。它适用于任何希望将测试用例用于自己的测试目的，或希望更深入了解测试用例创建方式的人员。
...
|------------|---------|---------|---------|
| CWE条目覆盖   | 119     | 118     | -0.8%   |
| 缺陷类型     | 1,489   | 1,617   | 8.6%    |
| 测试用例     | 57,099  | 61,387  | 7.5%    |
| 代码行数2    | 8,375,604 | 8,679,682 | 3.6%    |
...
| -------- | -------- |
| “CWE” 字符串文字 | 强制 |
| CWE ID | 与此测试用例相关的CWE条目的数字标识符，例如“36” | 强制 |
| “_” 字符串文字 | 强制 |
| 缩写CWE条目名称 | CWE条目名称的缩写版本，单词之间用下划线连接，例如“Absolute_Path_Traversal” | 强制 |
| “__” （两个下划线）字符串文字 | 强制 |
| 功能变体名称 | 描述此缺陷的特定变体的单词或短语，例如“fromConsole”。此项在第3.2节中进一步描述。 | 强制 |
| “_” 字符串文字 | 强制 |
| 流变体 | 描述测试用例复杂性类型的两位整数值，例如“01”、“02”或“61”。此项在第3.3节中进一步描述。 | 强制 |
| 子文件标识符 | 标识此文件在由多个文件组成的测试用例中的字符串，例如“a”、“b”、“_bad”、“_good1”。此项在第3.4.2节中进一步描述。 | 可选 |
| “.” 字符串文字 | 强制 |
| 语言标识符/文件扩展名 | 字符串文字“c”、“cpp”或“.h” | 强制 |
...
- std_testcase.h - 此头文件包含在每个C/C++测试用例源代码文件中，包含多个变量和宏定义。它还包括其他头文件，如“std_testcase_io.h”和系统头文件“stdio.h”，这样它们就不需要被每个测试用例包含。
输入/输出相关的支持文件：
